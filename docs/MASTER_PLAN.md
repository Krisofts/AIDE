...
- **Cara JDK & Android build-tools benar-benar dijalankan di Android belum diputuskan** — build resmi JDK/aapt2/d8 dari upstream (Temurin, Google) dikompilasi untuk Linux/Mac/Windows desktop (glibc), bukan untuk Android (bionic libc), jadi tidak bisa didownload & langsung dieksekusi apa adanya. **Keputusan ini satu paket dengan riset terminal di Phase 0.**

  - **Temuan riset publik #1 (SDK Manager, Juli 2026)**: berdasarkan README & issue publik `AndroidCSOfficial/android-code-studio` (GPLv3, satu lineage dengan AndroidIDE), pola yang dipakai bukan "custom installer untuk tiap komponen SDK", melainkan: bootstrap satu JDK yang bisa jalan + download Android *command-line tools* resmi dari Google (isinya kode Java portable + script launcher, jadi tidak terikat masalah bionic/glibc), lalu **pasang platform/build-tools/platform-tools lewat tool `sdkmanager` resmi itu sendiri** (dijalankan via Terminal), bukan reimplementasi sendiri. Mereka eksplisit mencatat NDK tidak bisa dipasang lewat jalur ini karena toolchain-nya tidak dikompilasi untuk Android.
  - **Keputusan desain yang sudah diterapkan**: `SdkComponent` punya field `installMethod` (`DIRECT_DOWNLOAD` vs `VIA_SDKMANAGER`). Hanya JDK & command-line tools dipasang langsung oleh AIDE; platform/build-tools/platform-tools ditandai `VIA_SDKMANAGER`, baru berfungsi setelah Terminal (Phase 3) selesai — AIDE tidak perlu membangun & memelihara mirror sendiri untuk setiap versi Android platform.
  - **Temuan riset publik #2 (bagaimana JDK-nya sendiri dijalankan, Juli 2026)**: ternyata AndroidIDE **tidak** memakai userland Linux penuh (Ubuntu/proot) untuk JDK — mereka pakai paket **native bionic** (OpenJDK dikompilasi khusus untuk Android, gaya Termux, lewat script installer `idesetup`). Tapi paket bionic itu **bukan dibangun sendiri oleh tim AndroidIDE** — mereka bergantung penuh pada satu maintainer pihak ketiga (dikreditkan sebagai "Lzhiyong"), dan repo distribusi resmi mereka untuk ini (`androidide-tools`) **sudah di-archive sejak Desember 2024** — bukti nyata bahwa pendekatan "bergantung satu maintainer eksternal" ini rapuh.
  - **Temuan riset publik #3 (kenapa Ubuntu/proot ala ACSIDE juga bermasalah)**: teknik `proot`/`proot-distro` yang dipakai untuk jalankan userland Linux penuh (kemungkinan besar dasar sandbox Ubuntu ACSIDE) **berlisensi GPLv3** — memakainya langsung akan menarik AIDE balik ke masalah copyleft yang sudah sengaja dihindari (sama seperti kenapa terminal-emulator Termux dihindari di §2). Lebih parah: **`proot` dilaporkan rusak di Android 15 tanpa root** — seccomp filter yang lebih ketat memblokir syscall `ptrace` yang jadi dasar kerja proot, menyebabkan crash `SIGSYS`. Ini kemungkinan besar **penjelasan sesungguhnya** di balik kegagalan setup ACSIDE yang dicatat di atas (poin "Setup wizard/onboarding rawan dead-end") — bukan sekadar bug UI, tapi environment Ubuntu/proot-nya sendiri yang gagal jalan di Android versi terbaru.
  - **Tiga opsi arsitektur yang tersisa, semua punya trade-off nyata**:
    1. *Ubuntu/proot penuh* (ala ACSIDE) — dukungan bahasa paling luas (Python/Dart/Clang lewat apt biasa), tapi `proot` GPLv3 & rusak di Android 15 tanpa root. **Saat ini tidak layak jadi fondasi.**
    2. *Bionic-native dari maintainer eksternal* (ala AndroidIDE klasik) — ringan, tidak kena masalah proot/seccomp sama sekali, tapi terbukti rapuh (single point of failure, repo distribusinya sudah mati).
    3. *Bionic-native dari repo paket yang aktif dipelihara* (mis. apt repo resmi `termux-packages` — proyek komunitas besar, bukan proot, bukan terminal-emulator Termux, murni sumber binary bionic yang sudah dikompilasi) — kemungkinan jalan tengah terbaik: tidak kena masalah proot/seccomp (biner native, bukan syscall translation), risiko staleness lebih rendah dari opsi 2 (dipelihara komunitas, bukan satu orang). **Perlu diverifikasi Phase 0**: lisensi & syarat konsumsi apt repo Termux oleh aplikasi pihak ketiga di luar app Termux itu sendiri (build script `termux-packages` sendiri umumnya permissive, tapi ketentuan pemakaian mirror binernya belum dikonfirmasi) — belum bisa dianggap otomatis aman sebelum dicek eksplisit.
  - **Rekomendasi sementara**: prioritaskan riset opsi 3 di Phase 0 sebagai kandidat utama, opsi 2 (build cross-compile bionic sendiri, bukan bergantung Lzhiyong) sebagai fallback kalau opsi 3 mentok di lisensi. Opsi 1 (proot) disimpan sebagai catatan, dipertimbangkan lagi hanya jika ada alternatif proot yang lisensinya bersih & kompatibel Android 15 di masa depan.
  - **Konfirmasi langsung di device nyata (Juli 2026)**: percobaan terhadap aplikasi lain di lineage yang sama (`com.tom.rv2ide`) menunjukkan opsi 3 **benar-benar jalan di produksi saat ini** — instalasi lewat `apt` yang menunjuk ke repo custom hasil hosting statis (GitLab Pages), tanpa proot/chroot/ekstraksi rootfs sama sekali, langsung memasang `openjdk-17` (bionic-native) & paket `androidide-tools` ke `/data/user/0/<package>/files/home`. Ini juga mengungkap fakta baru: walau repo GitHub resmi `androidide-tools` sudah di-archive, **paket binernya terus didistribusikan ulang oleh komunitas lewat mirror apt independen** — ekosistemnya "self-healing" terhadap kematian satu sumber, memperkuat kelayakan opsi 3 dibanding opsi 2.
  - **Implikasi untuk AIDE**: jangan hotlink ke repo pihak ketiga tersebut (infrastruktur mereka, di luar kendali & tidak pantas dipakai produk kompeting) — tapi ini bukti kuat bahwa membangun & meng-host repo apt bionic-native sendiri (mis. lewat GitHub Pages, cross-compile dari resep build yang lisensinya jelas) adalah jalan yang layak & sudah terbukti dipakai orang lain, bukan sekadar teori.
